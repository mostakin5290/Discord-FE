...
const OverviewTab = () => {
  ...
  const flags = currentServer?.systemMessageFlags || { welcomeMessage: true, boostMessage: true };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await dispatch(updateServerDetails({ serverId: currentServer.id, data: mergedData })).unwrap();
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
                  ...
                  <div className="w-[100px] h-[100px] rounded-full overflow-hidden bg-[#1e1f22] flex items-center justify-center group-hover:shadow-xl transition-all relative">
                      {imageUrl ? (
                         <img src={imageUrl} alt="Icon" className="w-full h-full object-cover" /> 
                          ...
                          <span className="text-3xl text-[#b5bac1]">{name.charAt(0).toUpperCase()}</span>
                      )}
                      <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <span className="text-[10px] font-bold text-white uppercase text-center">Change<br/>Icon</span>
                      </div>
                  </div>
             </div>
             <p className="text-[10px] text-[#949ba4] font-medium uppercase mt-1">Minimum Size: 128x128</p>
          </div>
      ...
      <div className="space-y-6">
           <h3 className="text-xs font-bold text-[#949ba4] uppercase">System Messages</h3>
           
           <div className="space-y-1.5">
                <Label className="text-[#dbdee1] font-medium text-sm">System Messages Channel</Label>
                <DropdownMenu>
                     <DropdownMenuTrigger className="flex w-full items-center justify-between rounded-[4px] px-3 py-2.5 text-sm bg-[#1e1f22] hover:bg-[#1e1f22] border-none text-[#dbdee1] outline-none">
                         <span>{currentServer?.channels?.find(c => c.id === systemChannelId)?.name ? `#${currentServer.channels.find(c => c.id === systemChannelId)?.name}` : "Select a channel"}</span>
                     ...
                     <DropdownMenuContent className="bg-[#1e1f22] border-[#2b2d31] text-[#dbdee1] w-[var(--radix-dropdown-menu-trigger-width)]">
                         {currentServer?.channels?.filter(c => c.type === 'TEXT').map(c => (
                             <DropdownMenuItem key={c.id} onClick={() => setSystemChannelId(c.id)} className="focus:bg-[#404249] focus:text-white cursor-pointer">
                                  #{c.name}
                             </DropdownMenuItem>
                         ))}
                     </DropdownMenuContent>
                </DropdownMenu>
                <p className="text-xs text-[#949ba4]">System messages will be sent to this channel.</p>
           </div>

           <div className="space-y-4 pt-2">
               {[
                   { label: "Send a random welcome message when someone joins this server.", checked: welcomeMessage, set: setWelcomeMessage },
                   { label: "Send a message when someone Boosts this server.", checked: boostMessage, set: setBoostMessage }
               ].map((item, i) => (
                   <div key={i} className="flex items-center justify-between">
                       <Label className="text-[#dbdee1] font-normal text-[14px]">{item.label}</Label>
                       <Switch 
      ...
      <div className="pt-6 pb-2">
         <div className="bg-[#111214] p-3 rounded-[5px] flex justify-between items-center transition-all">
             <span className="text-white text-sm font-medium pl-2">Careful — you have unsaved changes!</span>
             <div className="flex gap-3">
                 <Button variant="ghost" onClick={() => setName(currentServer.name)} className="text-white hover:underline h-8">Reset</Button>
                 <Button onClick={handleSave} disabled={isLoading} className="bg-[#248046] hover:bg-[#1a6334] text-white h-8 px-6 transition-colors">Save Changes</Button>
             </div>
         </div>
      </div>
    </div>
