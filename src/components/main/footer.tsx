...
import { LanguageDropdown } from "@/components/shared/dropdowns/language-dropdown";
import { AppLogo } from "@/components/main/app-logo";

export function Footer({
  footerStyle = "advanced",
  className = "",
}: FooterProps) {
        ...
        <div className="px-6 md:px-10 w-full max-w-[1260px] grid grid-cols-[repeat(4,1fr)] md:grid-cols-[repeat(8,1fr)] lg:grid-cols-[repeat(12,1fr)] gap-x-5">
          ...
          {footerMenuItems?.map((item, index) => (
            <div key={index} className="mb-10 col-span-2">
              <h3 className="pt-2 font-primary text-app-blurple text-[16px] leading-6 cursor-default">
                {item.title}
              </h3>
              {item?.links && (
        ...
        <div className="px-6 md:px-10 w-full max-w-[1260px] grid grid-cols-[repeat(4,1fr)] md:grid-cols-[repeat(8,1fr)] lg:grid-cols-[repeat(12,1fr)] gap-x-5">
          <div className="col-span-4 md:col-span-8 lg:col-span-12">
            ...
            <div className="flex justify-between items-center">
              ...
              <Link
                to={authButtonUrl}
                className="p-[7px_16px] inline-flex items-center bg-app-blurple hover:bg-app-blurple-hover text-app-white font-medium text-[14px] leading-6 no-underline hover:underline rounded-[40px] hover:shadow-[0_8px_15px_rgba(0,0,0,.2)] transition-colors duration-200 ease-in-out"
              >
                {authButtonText}
              </Link>
            </div>
        ...
        <div className="px-6 md:px-10 3xl:px-0 grid grid-cols-1 md:grid-cols-footer-grid-cols lg:grid-cols-footer-grid-cols-lg 2xl:grid-cols-footer-grid-cols-2xl gap-6 md:gap-x-[116px] md:gap-y-[62px] lg:gap-x-5 lg:gap-y-12 xl:gap-5">
          ...
          <div className="relative flex flex-col items-start col-span-1 row-span-1 md:col-span-3 md:row-span-6 lg:col-span-4 lg:row-span-1">
            ...
            <AppLogo
              ...
              width={59}
              height={44}
              linkClassName="mb-10 inline-block"
            />
            ...
            <div className="mt-12 hidden lg:block">
              <p className="mb-4 text-app-white/50 font-normal text-base leading-5">
                Social
              </p>
          ...
          {footerMenuItems?.map((item, index) => (
            <div
              key={index}
              className="hidden md:inline-block lg:block col-span-1 row-span-1 md:col-span-2 md:row-span-1 lg:col-span-2 lg:row-span-3"
            >
              <h3 className="mb-4 text-app-white/50 font-abcgintodiscord font-normal text-base leading-5">
                {item.title}
              </h3>
              {item?.links && (
          ...
          {footerMenuItems && footerMenuItems?.length > 0 && (
            <div className="md:hidden col-span-1">
              <div className="text-app-white/50 font-abcgintodiscord font-normal text-sm lg:text-base">
                Menu
              </div>
              <Accordion
                type="single"
              ...
              >
                {footerMenuItems?.map((item, index) => (
                  <AccordionItem
                    key={index}
                    value={item.title}
                    className="footer-menu-item-mobile block overflow-clip z-[1]"
                  >
                    {item?.links && item?.links?.length > 0 && (
                      <>
                        <AccordionTrigger className="py-6 text-app-white font-abcgintodiscord text-lg leading-5 hover:no-underline">
                          {item.title}
                        </AccordionTrigger>
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        footerStyle === "advanced" &&
          "my-0 py-4 md:py-0 first:pt-0 font-abcgintodiscord text-[14px] md:text-[16px]",
        className,
      )}
    >
      {children}
...
function SocialIcon({
  ...
  return (
    ...
    >
      <img
        src={icon || placeholderImageUrl}
        alt={alt ?? ""}
        width={24}
        height={footerStyle === "basic" ? 24 : 25}
        loading="lazy"
      />
    </Link>
  );
