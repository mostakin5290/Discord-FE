...
const CreateServerDialog = ({
  ...
  const templates = [
    ...
    { id: "school", label: "School Club", icon: BookOpen, color: "bg-orange-500" },
  ];
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[440px] bg-[#313338] text-white border-0 p-0 gap-0 overflow-hidden shadow-2xl rounded-lg">
            ...
            <div className="px-4 pb-4 space-y-2 max-h-[420px] overflow-y-auto custom-scrollbar">
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-semibold text-base tracking-wide">Create My Own</span>
                </div>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-[#313338] flex items-center justify-center">
                          <template.icon className={`w-6 h-6 transition-transform group-hover:scale-110 ${template.id === 'gaming' ? 'text-green-400' : template.id === 'friends' ? 'text-pink-400' : template.id === 'study' ? 'text-yellow-400' : 'text-orange-400'}`} />
                        </div>
                        <span className="font-semibold text-base tracking-wide">{template.label}</span>
                      </div>
                      <ChevronRight className="w-5 h-5 text-gray-400 group-hover:text-white transition-colors" />
                    </Button>
                  ))}
        ...
        {/* Customize Step */}
        {step === "customize" && (
          <form onSubmit={handleSubmit} className="flex flex-col h-full bg-[#313338] relative">
            ...
            <div className="p-6 space-y-6">
               <div className="flex justify-center">
                  {image ? (
                    <div className="relative group animate-in fade-in zoom-in duration-300">
                       <img src={image} className="w-[84px] h-[84px] rounded-full object-cover border-4 border-[#2B2D31] shadow-lg" />
                       <button
                          type="button"
                          onClick={() => setImage("")}
                    ...
                    <div {...getRootProps()} className="w-[84px] h-[84px] rounded-full border-2 border-dashed border-gray-500 bg-[#313338] flex flex-col items-center justify-center cursor-pointer hover:border-gray-300 transition-all relative group shadow-md hover:shadow-lg hover:bg-[#404249]">
                        <input {...getInputProps()} />
                        {loading ? <Spinner className="w-6 h-6 text-gray-400" /> : <Camera className="w-8 h-8 text-gray-400 group-hover:text-gray-200 transition-colors" />}
                        <div className="absolute -top-1 -right-1 bg-[#5865F2] rounded-full p-1 shadow-md border-[3px] border-[#313338] group-hover:scale-110 transition-transform">
               ...
               <div className="space-y-2">
                 <Label htmlFor="name" className="text-xs font-bold text-gray-300 uppercase tracking-wide ml-1">Server Name</Label>
                 <Input
                   id="name"
                   name="name"
                   value={name}
                 ...
                 <p className="text-[12px] text-gray-400 mt-1 ml-1 leading-snug">
                    By creating a server, you agree to Discord's <span className="text-[#00A8FC] cursor-pointer hover:underline">Community Guidelines</span>.
                 </p>
               </div>

               <div className="space-y-2">
                 <Label htmlFor="desc" className="text-xs font-bold text-gray-300 uppercase tracking-wide ml-1">Description (Optional)</Label>
                 <Textarea
                   id="desc"
                   name="desc"
                   value={desc}
