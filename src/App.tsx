    ...
    if (user) {
      ...
      if (socket) {
        ...
        socket.on("friend_request_cancelled", (payload) => {
             if (payload.requestId) dispatch(removePendingRequest(payload.requestId));
        });

        socket.on("friend_removed", (payload) => {
            if (payload.friendId) dispatch(removeFriendFromList(payload.friendId));
        });
        ...
        socket.on("user_connected", (payload) => {
            if (payload.userId) {
                dispatch(updateFriendStatus({ userId: payload.userId, status: "online" }));
            }
        });

        socket.on("user_disconnected", (payload) => {
            if (payload.userId) {
                dispatch(updateFriendStatus({ userId: payload.userId, status: "offline" }));
            }
        });
      }
    }
      ...
      <header>
        <Routes>
          ...
          <Route element={<ProtectedRoute />}>
            ...
            <Route path="/friends" element={<DirectMessagesPage />} />
