...
>(
  (
    ...
    const hasDropdownContent =
      ...
      dropdownContent?.links?.length > 0;
    const decorImage = dropdownContent?.decorImage
      ? "/images/decor" + dropdownContent.decorImage
      : placeholderImageUrl;

    if (hasDropdownContent) {
      const dropdownGridTemplateColClass =
        (dropdownContent.links?.length ?? 0) === 1
          ? "xl:grid-cols-[auto]"
          : (dropdownContent.links?.length ?? 0) === 2
            ? "xl:grid-cols-[auto_1fr]"
            : (dropdownContent.links?.length ?? 0) >= 3
              ? "lg:grid-cols-[auto_auto_auto] xl:grid-cols-[auto_1fr_auto]"
              : "";
            ...
            <div className="m-0 lg:mx-auto p-0 lg:p-8 xl:p-10 h-auto relative block bg-app-blurple rounded-none lg:rounded-[40px] xl:overflow-visible">
              ...
              >
                {dropdownContent?.links?.map((group, groupIndex) => (
                  <div
                    key={groupIndex + "-" + group.title + "-div"}
                    className="pb-0 border-b border-app-white/10 [border-bottom-style:none]"
                  >
                    <ul
                      key={groupIndex + "-" + group.title + "-ul"}
                      className={subMenuClass}
                    >
                      {group.title && (
                        <NavigationMenuListItemTitle title={group.title} />
                      )}
                      {group.subMenu?.map((item, itemIndex) => {
                        if (item.subMenu && item.subMenu.length > 0) {
                          return (
                            <div
                              key={itemIndex + "-" + item.title + "-subMenu"}
                            >
                              <li className={subMenuClass}>
                                {item.href ? (
                                  <NavigationMenuListItem item={item} />
                                ) : (
                                  <NavigationMenuListItemTitle
                                    title={item.title}
                                  />
                                )}
                                ...
                                <ul className={subMenuClass}>
                                  {item.subMenu.map((subItem, subItemIndex) => (
                                    <NavigationMenuListItem
                                      key={subItemIndex + "-" + subItem.title}
                                      item={subItem}
                                    />
                                  ))}
                                </ul>
                              </li>
                              {itemIndex < (group.subMenu?.length ?? 0) - 1 && (
                                <NavigationMenuDivider key={itemIndex} />
                              )}
                            </div>
                          );
                        }
                        ...
                        return (
                          <li key={itemIndex + "-" + item.title}>
                            <NavigationMenuListItem
                              item={item}
                              className={cn(!group.title && "mb-6 lg:mb-3")}
                            />
                          </li>
                        );
                      })}
              ...
              {dropdownContent?.decorImage && (
                <div
                  className={cn("nav-decor", dropdownContent?.decorImageClass)}
                >
                  <img
                    src={decorImage}
                    alt={"Decoration Image"}
                    className="animate-fade-in-up-custom w-full h-full object-cover absolute inset-0"
                  />
                </div>
              )}
            </div>
      ...
      <NavigationMenuItem ref={ref} className={className} {...props}>
        <NavigationMenuLink
          ...
          className={cn(
            ...
            navStyle === "basic" &&
              "m-2 xl:m-2.5 p-2 xl:p-2.5 !bg-transparent !text-app-white font-semibold text-[16px] no-underline hover:underline",
          )}
