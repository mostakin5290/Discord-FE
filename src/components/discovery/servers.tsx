        ...
        <>
          <div className="flex flex-col h-80 bg-[#555aff] items-center justify-between px-10">
              ...
              <div className="flex items-center gap-2">
                {serverCategories.map((category, idx: number) => (
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="text-white text-[14px] bg-transparent hover:bg-transparent border-none"
                  >
                    <span className="text-white text-[14px] hover:underline transition-all duration-300 cursor-pointer">
                      {category.label}
                    </span>
                  </Button>
                ))}
            ...
            <div className="mx-auto xl:mx-0 pb-1.5 pr-0 lg:pr-8 2xl:pr-0 lg:max-w-[unset] relative w-full mb-20">
              <h2 className="heading--h1 is_2024 leading-0">
                {serverCategories.find((val) => val.label === category)?.title}
              </h2>
              <p className="text-[16px] font-sans tracking-tight font-medium text-[#e0e0e0] leading-0">
                {
                  serverCategories.find((val) => val.label === category)
                    ?.catlogSubtitle
                }
              </p>
            </div>
          ...
          <div className="flex flex-col h-full bg-[#1b1b1b] rounded-lg p-4 gap-4">
            <h3 className="font-bold text-white text-[18px]">
              {serverCategories.find((val) => val.label === category)?.subtitle}
            </h3>
            {/* Server Cards */}
            ...
            <div className="grid grid-cols-4 gap-4">
              {featuredServers.map((server, idx: number) => (
                <div
                  className="w-70 bg-[#26272c] rounded-[8px] overflow-hidden shadow-lg"
                  key={idx}
                >
                  {/* Banner Image */}
          ...
          <div className="flex flex-col h-full bg-[#1b1b1b] rounded-lg p-4 gap-4">
            <div className="flex items-center justify-start w-full h-10 border-b border-[#d8d8d8] px-3 gap-5">
              <Button
                ...
                variant="outline"
                size="sm"
                className="text-white text-[14px] bg-transparent border-none hover:text-[#dbdee1] transition-all duration-200 hover:scale-103 active:scale-95 hover:bg-[#26272c]"
              >
            ...
            <div className="mx-auto xl:mx-0 pb-1.5 pr-0 lg:pr-8 2xl:pr-0 lg:max-w-[unset] relative w-full mb-5">
              ...
              <p className="text-[16px] font-sans tracking-tight font-medium text-[#e0e0e0] leading-0">
                ...
                {searchResults.length > 1 ? "servers" : "server"}
              </p>
            </div>
            {isLoading ? (
              ...
              <div className="grid grid-cols-4 gap-4">
                {searchResults.length > 0 ? (
                  searchResults.map((server, idx: number) => (
                    <div
                      className="w-70 bg-[#26272c] rounded-[8px] overflow-hidden shadow-lg"
                      key={idx}
                    >
