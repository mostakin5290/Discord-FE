...
import { Search, Shield, Plus, Trash2, UserPlus } from "lucide-react";
...
const CustomRoles = () => {
  ...
  const handleDeleteRole = async (roleId: string) => {
      ...
      try {
          ...
          if (selectedRole?.id === roleId) setSelectedRole(roles.find((r: any) => r.id !== roleId) || null);
      } catch (error) {
  ...
  const filteredRoles = roles ? roles.filter((role: any) => 
  ...
  const hasChanges = () => {
    ...
    // Sort logic
    const currentPerms = [...rolePermissions].sort();
    let initialPerms: string[] = [];
    try {
        initialPerms = [...(JSON.parse(selectedRole.permissions || "[]"))];
    } catch(e) { initialPerms = [] }
    initialPerms.sort();

    return (
        roleName !== selectedRole.name ||
        roleColor !== selectedRole.color ||
        JSON.stringify(currentPerms) !== JSON.stringify(initialPerms)
    );
  };
                      ...
                      <div className="max-w-2xl space-y-8 animate-in fade-in duration-300">
                          ...
                          {activeSubTab === 'members' && (
                              <div className="flex flex-col gap-4 text-center py-10">
                                  <div className="w-16 h-16 rounded-full bg-[#2b2d31] flex items-center justify-center mx-auto mb-2">
                                      <UserPlus className="w-8 h-8 text-[#949ba4]" />
                                  </div>
                                  <h3 className="text-xl font-bold text-white">Manage Members</h3>
                                  <p className="text-[#949ba4] max-w-sm mx-auto">
                                      View and manage members assigned to this role. You can verify who has this role from the Members tab.
                                  </p>
                                  <Button variant="secondary" className="mx-auto mt-4 bg-[#4e5058] hover:bg-[#6d6f78] text-white">
                                      View Members
                                  </Button>
                              </div>
                          )}
